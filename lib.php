...
function theme_boost_union_get_main_scss_content($theme) {
    ...
    if (\theme_boost_union\local\mwp::extension_present() == true) {
        ...
        if (function_exists('theme_workplace_get_main_scss_content')) {
            // Get and include the main SCSS from Boost Workplace.
 ...
 * @param bool $skipmwpcheck Whether to skip the check if we are on Moodle Workplace.
...
function theme_boost_union_alter_css_urls(&$urls, $skipmwpcheck = false) {
    ...
    // If we are on Moodle Workplace (and the MWP check is not skipped).
            ...
            if ($urls[$i] instanceof \core\url) {
                ...
                if (preg_match("|^$pathstyles(/_s)?(.*)$|", $urls[$i]->out(false), $matches)) {
                    ...
                    if ($noslashargumentsallowed == true) {
                        ...
                        // Add flavourid before the file type segment.
...
/**
 * Callback for Moodle Workplace tenant get css config.
...
function theme_boost_union_tenant_get_css_config(array &$info, int $tenantid, array $filemanageroptions): void {
    ...
    // Additionally, if we are not on Moodle Workplace, return directly as well.
...
/**
 * Callback to add extra element to Moodle Workplace tenant branding settings.
...
/**
 * Callback for Moodle Workplace tenant CSS form submission.
